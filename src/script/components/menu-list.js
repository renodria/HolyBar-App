...
class ItemList extends HTMLElement {
  ...
  async render() {
    ...
    categories.forEach((category) => {
      categoryList += `<li class="p-2 transition-all hover:bg-accent hover:text-white cursor-pointer">${category.strCategory}</li>`;
    });
    this.innerHTML = `
    <div class="pb-4 pl-4 md:pt-0 sm:pt-0 flex items-center justify-between">
        <h1
          id="category-list-up"
          class="lg:text-xl md:text-lg text-base font-bold"
        >
          Menu List
        </h1>
      </div>
    ...
    const meals = await DataSource.getMealByCategory("Breakfast");
    let mealCards = "";
...
customElements.define("menu-list", ItemList);
